...
Not yet deployed. `vars.ORIGIN_URL` in `wrangler.jsonc` is still the placeholder
`https://door-sync.example.org` — it must point at the Pi's real tunnel hostname before any
deploy. See README.md for the full setup sequence (create queues, set secrets, deploy).
...
- **`wrangler.jsonc`** — all deploy/runtime config: the `EVENTS` queue producer, the consumer
  (`max_batch_size` 10, `max_retries` 5, `retry_delay` 30s, DLQ `door-webhook-dlq`), and the
  non-secret `ORIGIN_URL` var. `nodejs_compat` is on and `observability` is enabled.
- **Secrets** (`CIVICRM_WEBHOOK_SECRET`, `ORIGIN_HMAC_SECRET`, `CF_ACCESS_CLIENT_ID`,
  ...
  Locally they live in `.dev.vars`, which is git-ignored and which the test pool loads.
